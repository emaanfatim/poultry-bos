...
interface ReceiptPreviewProps {
  ...
  onPrint: () => void;
  onNewSale: () => void;
  // Hidden when this component is reused to view a past receipt (e.g. from
  // the daily summary) rather than right after checkout.
  showNewSale?: boolean;
}
...
export function ReceiptPreview({
  ...
  // Bill type was already decided by the cashier at checkout (Priced Invoice
  // vs Unpriced Invoice) — the receipt just reflects that choice, it isn't
  // re-selectable here.
  ...
  // Charges (tax/surcharge/other) — per_product-scope charges produce one
  // row per line item they applied to, so group by category name and sum
  // for a single receipt line per charge (e.g. one "GST" line, not one per
  // product). Percentage-based charges also carry their rate so the
  // receipt can show e.g. "GST (18%)" alongside the amount.
  ...
  function chargeLabel(charge: { name: string; calculationType: "fixed" | "percentage"; rateValue: string }) {
  ...
  // GST/other charges show their rate inline (e.g. "GST (18%)"), so mirror
  // that for the discount line too, deriving the rate from the amount that
  // was actually applied against the pre-discount subtotal.
  const discountLabel = (() => {
  ...
  // Subtotal after discount is applied, before any charges (GST/surcharge)
  // are added on top.
  ...
  function buildPdf() {
    ...
    if (isPriced) {
      if (hasDiscount) {
        ...
        text(formatCurrency(transaction.subtotal, symbol), pageWidth - margin, 10, "right");
        y -= 14;
        text(discountLabel, margin, 10);
        text(`- ${formatCurrency(transaction.discountAmount!, symbol)}`, pageWidth - margin, 10, "right");
        y -= 14;
        text(t.receipt.subtotalAfterDiscount, margin, 10);
        text(formatCurrency(subtotalAfterDiscountNumber.toFixed(2), symbol), pageWidth - margin, 10, "right");
        y -= 4;
      }
      if (aggregatedCharges.length > 0 || hasRounding) {
        ...
        for (const charge of aggregatedCharges) {
          ...
          text(formatCurrency(charge.amount.toFixed(2), symbol), pageWidth - margin, 10, "right");
          y -= 14;
        }
        if (hasRounding) {
        ...
        {isPriced && (
          <div className="mt-4 border-t border-slate-200 pt-4">
            ...
            {aggregatedCharges.map((charge) => (
              <div
                key={charge.name}
                className="mt-1 flex justify-between text-sm text-slate-500"
              >
