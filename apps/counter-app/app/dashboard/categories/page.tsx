...
export default function CategoriesPage() {
  ...
  async function load() {
    if (!token) return;
    try {
      const data: Category[] = await apiFetch("/categories", token);
      setCategories(data);
      setSubForms((prev) => {
        const next = { ...prev };
  ...
  // ─── Block non-owners AFTER all hooks ────────────────────────────────────
  ...
  async function createCategory() {
    if (!newCatName.trim() || !newCatToken.trim() || !token) return;
    setCatSaving(true);
    setError("");
    try {
      await apiFetch("/categories", token, {
        method: "POST",
  ...
  async function deleteCategory(id: string) {
    if (!confirm("Delete this category and all its subcategories?") || !token) return;
    setError("");
    try {
  ...
  async function createSubCategory(catId: string) {
    ...
    if (!form?.name.trim() || !form?.token.trim() || !token) return;
    updateSubForm(catId, { saving: true });
    setError("");
    try {
      await apiFetch("/categories/subcategories", token, {
        method: "POST",
        body: JSON.stringify({
          categoryId: catId,
          name: form.name.trim(),
  ...
  async function deleteSubCategory(id: string) {
    if (!confirm("Delete this subcategory?") || !token) return;
    setError("");
    try {
      await apiFetch(`/categories/subcategories/${id}`, token, { method: "DELETE" });
      await load();
    } catch (e: unknown) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mb-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="mb-4 text-base font-semibold text-slate-800">New Category</h2>
              ...
              <div key={cat.id} className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div className="px-5 py-3">
                  {cat.subCategories.length === 0 ? (
                    <p className="py-2 text-xs text-slate-400">No subcategories yet.</p>
                  ) : (
                  ...
                  {!form.open ? (
                    ...
                    <div className="flex gap-2 pt-1">
                      <input
                        ...
                        value={form.name}
                        onChange={(e) => updateSubForm(cat.id, { name: e.target.value })}
                      ...
                      <input
                        ...
                        value={form.token}
                        onChange={(e) => updateSubForm(cat.id, { token: e.target.value })}
                      ...
                      <button
                        ...
                        disabled={form.saving || !form.name.trim() || !form.token.trim()}
                      ...
                      <button
                        type="button"
                        onClick={() => updateSubForm(cat.id, { open: false, name: "", token: "" })}
