...
// Discount module — self-service lookup so the POS can show the cashier
// exactly which products they're approved to discount, before checkout.
export async function fetchMyDiscountSettings(
  token: string,
): Promise<CashierDiscountSettings> {
  const data = await api.get<{ settings: CashierDiscountSettings }>(
    "/discount-settings/me",
    token,
  );
