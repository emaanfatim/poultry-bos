...
import type { AuthUser, TenantConfig } from "@repo/types";
import { getMe, login as loginRequest } from "../services/auth";

const STORAGE_KEY = "bos_owner_session";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    getMe(stored.token)
      .then((data) => {
        if (!data.user || !data.tenant || !data.branch) {
  ...
  const login = useCallback(async (username: string, password: string) => {
    ...
    if (data.user.role !== "owner") {
      throw new Error("Only owner accounts can access the Owner Portal");
    }
