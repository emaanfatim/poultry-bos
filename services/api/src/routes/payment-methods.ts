...
const createSchema = z.object({
  ...
  roundingMethod: z.enum(["exact", "round_up", "round_down"]).optional().default("exact"),
});

const updateSchema = z.object({
  name: z.string().min(1).optional(),
  requiresRounding: z.boolean().optional(),
  roundingMethod: z.enum(["exact", "round_up", "round_down"]).optional(),
...
paymentMethodRoutes.post("/", requireOwner, async (c) => {
  ...
  const [method] = await db
    ...
    .values({
      ...
      roundingMethod: parsed.data.roundingMethod,
    })
    .returning();
