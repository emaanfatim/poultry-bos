...
import { roundMoney, startOfToday, endOfToday, todayDateKey } from "../lib/money";
...
// top of the opening float. Only "priced" bills are counted: an "unpriced"
// (delivery note) bill isn't paid in cash at the counter, so it never
// touches the till.
async function computeCashInDrawer(
  tenantId: string,
  branchId: string,
  userId: string,
  openedAt: Date,
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(transactions.billType, "priced"),
        gte(transactions.createdAt, openedAt),
        lt(transactions.createdAt, asOf),
      ),
    );
...
export interface RoundingSummary {
  // Sum of positive roundingAdjustment — bills where the system rounded UP
  // (or a custom amount above trueTotal) and the till collected more than
  // the itemized total. This is cash the business received on top of the
  // bill, not a cashier error.
  extraReceived: number;
  // Sum of |negative roundingAdjustment| — bills rounded DOWN (or a custom
  // amount below trueTotal), where the till collected less than the
  // itemized total. This is the "gave away change we didn't have exact
  // notes for" case.
  extraGiven: number;
  // extraReceived - extraGiven. Matches exactly what shifts
  // expectedClosingCash away from the sum of trueTotal — i.e. this fully
  // explains any gap between "what the bills add up to" and "what the
  // drawer should hold", so it is not a source of till variance by itself.
  net: number;
  transactionCount: number;
}

// Same eligibility window as computeCashInDrawer (cash, priced, completed —
// refunds are excluded since a refunded sale's rounding never took real
// cash across the counter for the remaining shift). Purely a breakdown of
// data already stored on each transaction; does not change till math.
async function computeRoundingSummary(
  tenantId: string,
  branchId: string,
  userId: string,
  openedAt: Date,
  asOf: Date,
): Promise<RoundingSummary> {
  const db = getDb();
  ...
  for (const row of rows) {
    const adjustment = parseFloat(row.roundingAdjustment ?? "0");
    if (adjustment === 0) continue;
    counted += 1;
    if (adjustment > 0) extraReceived += adjustment;
    else extraGiven += Math.abs(adjustment);
  }

  return {
    extraReceived: roundMoneyNumber(extraReceived),
    extraGiven: roundMoneyNumber(extraGiven),
    net: roundMoneyNumber(extraReceived - extraGiven),
...
tillRoutes.get("/current", async (c) => {
  ...
  const roundingSummary = await computeRoundingSummary(
    tenantId,
    session.branchId,
    session.userId,
    session.openedAt,
...
tillRoutes.post("/close", async (c) => {
  ...
  return c.json({
    session: {
      ...formatSession(updated!, user.displayName, counts),
      roundingSummary,
    },
  });
});
...
tillRoutes.get("/report", async (c) => {
  ...
  const reportDateKey = todayDateKey();
  return c.json({
    summary: {
      date: `${reportDateKey.slice(0, 4)}-${reportDateKey.slice(4, 6)}-${reportDateKey.slice(6, 8)}`,
      rows: reportRows,
