...
//
// `override_rate` (extension — owner-controlled per-product rate, not just
// on/off) behaves like `override_on` for applicability purposes, but also
// carries its own calculationType + value forward as the "rate override"
// for every more-specific level, until either a more specific row clears it
// (override_on/override_off) or replaces it (another override_rate). This
// lets a Charge Category stay at its normal rate everywhere except the one
// target an owner has explicitly given a different value — e.g. Tax at 15%
// everywhere, 5% only on "Leg Piece" — without needing a second category.
...
export interface ResolvedCategoryForProduct {
  ...
  // Set when the most specific level with an explicit row for this category
  // is `override_rate` — the category's normal rate line(s) should be
  // ignored in favor of this value (see selectApplicableRateLine callers).
...
export async function resolveChargeCategoriesForProduct(
  ...
  const rateOverrideByCategoryId = new Map<string, RateOverride>();
  for (const [categoryId, rows] of byCategory) {
    let applies = false;
    let rateOverride: RateOverride | null = null;
    for (const level of LEVEL_ORDER) {
      const row = rows.find((r) => r.assignmentLevel === level);
      if (!row) continue; // no explicit row at this level → inherit
      if (row.overrideType === "override_on") {
        applies = true;
        rateOverride = null;
      } else if (row.overrideType === "override_off") {
        applies = false;
        rateOverride = null;
      } else if (row.overrideType === "override_rate") {
        applies = true;
        rateOverride = {
          calculationType: row.rateOverrideCalculationType as "fixed" | "percentage",
          value: row.rateOverrideValue as string,
        };
      }
      // 'inherit' explicitly carries the previous state forward — no-op
    }
    if (applies) {
      applicableCategoryIds.push(categoryId);
      if (rateOverride) rateOverrideByCategoryId.set(categoryId, rateOverride);
    }
  }
  ...
  return categories.map((category) => ({
    ...
    rateOverride: rateOverrideByCategoryId.get(category.id) ?? null,
  }));
}
...
/**
 * Applies a resolved product/category-level rate override onto the rate
 * line that would otherwise apply. Payment-method / manual-selection
 * conditioning is still honored — the override only replaces the *value*,
 * never the conditional-selection behavior — so `line` should already be
 * the output of `selectApplicableRateLine`.
 */
export function applyRateOverride(
  line: ChargeRateLineLike,
  rateOverride: RateOverride | null,
): ChargeRateLineLike {
  if (!rateOverride) return line;
  return {
    ...line,
    calculationType: rateOverride.calculationType,
    value: rateOverride.value,
  };
}
...
chargeResolutionRoutes.get("/product/:productId", async (c) => {
  ...
  return c.json({
    charges: resolved.map(({ category, rateLines, rateOverride }) => {
      ...
      const effective = applicable ? applyRateOverride(applicable, rateOverride) : null;
      return {
        category: {
          id: category.id,
          name: category.name,
        ...
        // Set when an owner has given THIS product (or its category/
        // sub-category/branch) its own rate for this Charge Category —
        // the value that will actually be billed instead of the normal
        // rate line's value/calculationType.
        rateOverride,
        effectiveCalculationType: effective?.calculationType ?? null,
